...
# Set up talker
if params["talker"].lower() == "terminal":
    talker = TerminalTalker(
        language=params.get("language","en"),
        prefix=params.get("terminal_talker_prefix", "\nAssistant: ").format(**params)
    )
elif params["talker"].lower() == "speaker":
    talker = LocalTalker(
        language=params.get("language","en"),
    )
elif params["talker"].lower() == "nao":
    talker = NAOTalker(
        ip=params["ip"],
        language=params.get("language","en"),
    ...
    raise Exception("Incorrect 'talker' specified! Use 'terminal', 'speaker' or 'NAO'")

# Set up listener
if params["listener"].lower() == "mic":
    listener = Listener(
        language=params.get("language","en"),
        default_mic=params.get("default_mic",True),
        use_whisper=params.get("use_whisper",False)
    )
elif params["listener"].lower() == "terminal":
    listener = lambda : input(params.get("terminal_listener_prefix","User: "))
elif params["listener"].lower() == "timer":
